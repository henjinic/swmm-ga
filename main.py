...
def main():
    _, mask, area_map, _, road_area_map, neargreen_mask, direction_vectors, original_areas = load_site_data("sub_og.csv")
    ...
    best = ga.run(size=100, strategy="cost", child_count=100, mutation_rate=0.9, stable_step_for_exit=20, is_logging=False)
    # best = ga.run(size=40, strategy="age", elitism=4, mutation_rate=0.9, stable_step_for_exit=50, is_logging=False)
    # best = ga.run(size=8, strategy="age", elitism=2, mutation_rate=0.9, stable_step_for_exit=5, is_logging=False)
